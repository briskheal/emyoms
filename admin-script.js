...
async function downloadAdminPDCN_PDF() {
    ...
    const tableData = (currentPDCNReviewItems || []).map(i => {
        const billed = parseFloat(i.billedPrice || 0);
        const special = parseFloat(i.specialPrice || 0);
        const gstPct = parseFloat(i.gstPercent || 0);
        const marginPct = parseFloat(i.marginPct || 10);
        ...
        return [
            ...
            billed.toFixed(2),
            special.toFixed(2),
            (unitDiffIncl * qty).toFixed(2),
            (unitMargin * qty).toFixed(2),
            finalItemPDCN.toFixed(2)
        ];
    });

    doc.autoTable({
        startY: 110,
        head: [['Product Name', 'Qty', 'GST %', 'Billed Rate', 'Spl Rate', 'Diff+GST', 'Margin', 'Final PDCN']],
        body: tableData,
        theme: 'grid',
