...
// Target 1: openPaymentModal
const openModalRegex = /function openPaymentModal\(\) \{[\s\S]*?modal\.classList\.remove\('hidden'\);\n\}/;
...
if (openModalRegex.test(script)) {
    script = script.replace(openModalRegex, replace1);
    console.log('Replaced openPaymentModal');
}

// Target 2: updatePaymentContext
const updateCtxRegex = /badge\.innerText = type === 'RECEIPT' \? "RECEIPT" : "PAYMENT OUT";[\s\S]*?vPlaceholder\.innerText = `PAY\$\{type === 'RECEIPT' \? 'IN' : 'OUT'\}-\$\{yearTag\}-XXXX`;/;
...
if (updateCtxRegex.test(script)) {
    script = script.replace(updateCtxRegex, replace2);
    console.log('Replaced updatePaymentContext');
}
