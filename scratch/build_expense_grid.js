...
// === 1. Replace the "Expense Management" card in Reports tab to include a list view ===
const oldCard = `                    <!-- EXPENSE REPORTS -->
                    <div class="glass-card" style="padding: 1.5rem; border-top: 4px solid #ef4444;">
                        <h3 class="section-title" style="font-size: 1rem; margin-bottom: 1.5rem;"><span style="opacity: 0.8;">??</span> Expense Management</h3>
                        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                            <button class="btn btn-ghost" style="justify-content: flex-start; text-align: left;" onclick="generateReport('expense-transaction')">
                                <div style="display: flex; flex-direction: column;">
                                    <span style="font-size: 0.85rem; font-weight: 800; color: #fff;">Expense Transaction Report</span>
                                    <span style="font-size: 0.65rem; color: var(--text-muted);">Log of all operational outgoings.</span>
                                </div>
                            </button>
                            <button class="btn btn-ghost" style="justify-content: flex-start; text-align: left;" onclick="generateReport('expense-category')">
                                <div style="display: flex; flex-direction: column;">
                                    <span style="font-size: 0.85rem; font-weight: 800; color: #fff;">Expense Category Report</span>
                                    <span style="font-size: 0.65rem; color: var(--text-muted);">Expenses grouped by Direct and Indirect headings.</span>
                                </div>
                            </button>
                            <button class="btn btn-ghost" style="justify-content: flex-start; text-align: left;" onclick="generateReport('expense-item')">
                                <div style="display: flex; flex-direction: column;">
                                    <span style="font-size: 0.85rem; font-weight: 800; color: #fff;">Expense Item Report</span>
                                    <span style="font-size: 0.65rem; color: var(--text-muted);">Granular breakdown of specific expense items.</span>
                                </div>
                            </button>
                            
                            <button class="btn btn-primary" style="margin-top: 1rem;" onclick="openExpenseModal()">
                                ? LOG NEW EXPENSE
                            </button>
                        </div>
                    </div>`;
...
if (html.includes(oldCard)) {
    ...
    console.log('SUCCESS: Expense card replaced.');
} else {
    // Try index-based
    const marker = '                    <!-- EXPENSE REPORTS -->';
    const start = html.indexOf(marker);
    const endMarker = '                    </div>\n                </div>';
    // find the end of expense card section
    const cardEnd = html.indexOf('</div>\n                </div>', start);
    if (start >= 0 && cardEnd >= 0) {
        const fullEnd = cardEnd + '</div>\n                </div>'.length;
        html = html.slice(0, start) + newCard + html.slice(fullEnd);
        console.log('SUCCESS via index. Start:', start);
    } else {
        console.log('ERROR: Could not find expense card. Start found:', start >= 0);
    }
}
    ...
    <div id="editExpenseModal" class="hidden" style="position: fixed; inset: 0; background: rgba(0,0,0,0.92); backdrop-filter: blur(20px); display: flex; align-items: center; justify-content: center; z-index: 5000; padding: 1rem;">
        <div class="glass-card" style="width: min(95vw, 650px); margin: 0; border-top: 5px solid #f59e0b; padding: 0; overflow: hidden; border-radius: 20px;">
            <div style="background:rgba(255,255,255,0.03); padding:1rem 1.5rem; border-bottom:1px solid var(--glass-border); display:flex; justify-content:space-between; align-items:center;">
                ...
                <button onclick="document.getElementById('editExpenseModal').classList.add('hidden')" style="width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);color:#fff;font-size:1rem;cursor:pointer;">&#10005;</button>
            </div>
            <form id="editExpenseForm" onsubmit="updateExpense(event)" style="padding:1.5rem;">
                ...
                <div style="display:flex; justify-content:space-between; border-top:1px solid var(--glass-border); padding-top:1rem;">
                    ...
                    <div style="display:flex; gap:0.75rem;">
                        <button type="button" onclick="document.getElementById('editExpenseModal').classList.add('hidden')" style="padding:9px 20px; background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:#fff; border-radius:8px; cursor:pointer; font-size:0.8rem;">CANCEL</button>
...
console.log('DONE. HTML size:', html.length);
