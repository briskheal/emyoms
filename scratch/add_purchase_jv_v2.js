...
async function autoPostPurchaseJV(entry, stockist) {
    ...
    if (purchaseVal > 0) {
        lines.push({ jvId: jv.id, type: 'DR', amount: purchaseVal, entityType: 'Ledger', entityId: stockLedger ? stockLedger.id : null, entityName: 'Opening Stock', notes: \`Purchase Value + Charges\` });
    }

    const gstVal = Number(entry.gstAmount || 0);
    if (gstVal > 0) {
        lines.push({ jvId: jv.id, type: 'DR', amount: gstVal, entityType: 'Ledger', entityId: gstLedger ? gstLedger.id : null, entityName: 'GST Payable', notes: \`Input GST\` });
    }

    lines.push({ jvId: jv.id, type: 'CR', amount: entry.grandTotal, entityType: 'Stockist', entityId: stockist ? stockist.id : null, entityName: stockist ? stockist.name : 'Unknown Supplier', notes: \`Invoice \${entry.supplierInvoiceNo || ''}\` });
