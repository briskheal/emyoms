...
async function reconcile() {
    ...
    async function postJV(refType, refId, date, amount, narration, drLines, crLines) {
        ...
        if (existingRefIds.includes(refKey)) {
        ...
        try {
            ...
            const jv = await db.JournalVoucher.create({
                jvNo, date, narration, totalAmount: amount, refType, refId
            }, { transaction: t });

            const lines = [];
            drLines.forEach(l => lines.push({ ...l, jvId: jv.id, type: 'DR' }));
            crLines.forEach(l => lines.push({ ...l, jvId: jv.id, type: 'CR' }));
